...
try:
    ...
    from src.widgets.indicator_settings_dialog import IndicatorSettingsDialog
    ...
    from widgets.indicator_settings_dialog import IndicatorSettingsDialog
...
class SmartStockMainWindow(QtWidgets.QMainWindow):
    """SmartStock 純原生 Qt6 量化桌面主視窗 (Pure Native Desktop Application v1.0.41)"""
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SmartStock 智慧型量化交易與選股平台 v1.0.41 (Pure Native Qt6)")
    ...
    def _build_ui(self):
        ...
        title_label = QtWidgets.QLabel("📈 SmartStock 量化交易 v1.0.41")
    ...
    def _build_market_overview_tab(self):
        ...
        btn_indicator = QtWidgets.QPushButton("⚙️ 技術指標設定")
        btn_indicator.setStyleSheet("background-color: #0066FF; color: #FFFFFF; font-weight: bold; padding: 4px 12px;")
        btn_indicator.clicked.connect(self.open_indicator_settings)
        kline_header.addWidget(btn_indicator)
    ...
    def open_indicator_settings(self):
        """彈出技術指標與顏色詳細設定視窗"""
        dlg = IndicatorSettingsDialog(self.chart_widget.indicator_config, self)
        dlg.config_saved_signal.connect(self.chart_widget.set_indicator_config)
        dlg.exec()
